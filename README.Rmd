...
The heatmap functions use the `@netP$prob` slot from CellChat objects. The
examples below use tiny CellChat-like S4 objects with only that slot so the
README can render quickly.
...
```{r single-sample-heatmap, fig.width = 8, fig.height = 5}
result <- makehm(
...
```{r pooled-heatmap, fig.width = 8, fig.height = 5}
...
```{r differential-heatmap, fig.width = 8, fig.height = 5}
...
```{r custom-differential-heatmap, fig.width = 8, fig.height = 5}
